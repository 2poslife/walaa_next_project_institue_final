...
export async function POST(
  ...
  try {
    ...
    if (!lesson.deleted_at) {
      return successResponse({}, 'Remedial lesson is already not deleted');
    }
    ...
    if (updateError) {
      return errorResponse('Failed to restore remedial lesson');
    }

    return successResponse({}, 'Remedial lesson restored successfully');
  } catch (error) {
    console.error('Restore remedial lesson error:', error);
